...
import java.util.*; // Used for keyboard input scanner
...
public class main {
    ...
    public static void main(String[] args) {
        ...
        try {
            while (menuChoice != 'Q') {
                ...
                if (menuChoice == 'A') {
                    ...
                    System.out.printf("%n%s%n", "Please enter the student's " +
                    "name followed by a space and their academic year.");
                    name = kbScan.next();
                    year = kbScan.nextInt();

                    // Prompt user for student classes
                    System.out.printf("%n%s%n", "Please enter the student's clas" +
                    "ses with each separated by a space.");
                    for (int ct1 = 0; ct1 < CLASS_CAPACITY; ct1++) {
                        classes.insert(kbScan.next());
                    }

                    // Prompt user for student grades
                    System.out.printf("%n%s%n", "Please enter the student's grades " +
                ...
                // Delete student
                    ...
                    // Print student average for requested grade index
                    System.out.printf("%n%s%n", "Average: " + gradeAvg / studentDB.size());
                }
                // Inform user of invalid menu option
                else {
        ...
        // Catch any attempts at accessing invalid index
        catch (NullPointerException e) {
            System.out.println("Bad index, check your input");
        }
    }
}
